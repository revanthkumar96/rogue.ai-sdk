...
class TestLoggerInitialization(unittest.TestCase):
    ...
    def test_yaml_config_affects_logger_initialization(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('rouge.utils.config.Path.cwd',
                ...
                self.assertEqual(logger.config.service_name,
                                 'logger-test-service')
                ...
                self.assertEqual(logger.config.github_commit_hash,
                                 'loggercommit123')
    ...
    def test_init_overrides_affect_logger_config(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('rouge.utils.config.Path.cwd',
                ...
                self.assertEqual(logger.config.service_name,
                                 'override-logger-service')  # Overridden
                self.assertEqual(logger.config.environment,
                                 'override-env')  # Overridden
                self.assertEqual(logger.config.github_owner,
                                 'yaml-owner')  # From YAML
                self.assertEqual(logger.config.github_repo_name,
                                 'yaml-repo')  # From YAML
                self.assertEqual(logger.config.github_commit_hash,
                                 'yamlloggercommit')  # From YAML
                self.assertEqual(logger.config.enable_log_cloud_export,
                                 False)  # Overridden
                self.assertEqual(logger.config.aws_region,
    ...
    def test_logger_without_yaml_config(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('rouge.utils.config.Path.cwd',
                ...
                # Verify that logger configuration comes from init()
                # parameters only
                self.assertEqual(logger.config.service_name,
                                 'logger-init-only')
    ...
    def test_get_logger_before_initialization_raises_error(self):
        """Test that get_logger() raises error when
    ...
    def test_get_logger_with_custom_name(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with patch('rouge.utils.config.Path.cwd',
                ...
                self.assertEqual(default_logger.config.service_name,
