...
class TestEnvironmentVariables(unittest.TestCase):
    ...
    def test_boolean_env_var_parsing(self):
        ...
        for env_value, expected_bool in test_cases:
            with self.subTest(env_value=env_value, expected=expected_bool):
                os.environ['ROUGE_LOCAL_MODE'] = env_value
                env_config = tracer._load_env_config()
                self.assertEqual(env_config['local_mode'], expected_bool)
                del os.environ['ROUGE_LOCAL_MODE']
    ...
    def test_tracer_verbose_env_var(self):
        """Test that ROUGE_TRACER_VERBOSE environment variable works "
        "correctly"""
        ...
        for env_value, expected_bool in test_cases:
            with self.subTest(env_value=env_value, expected=expected_bool):
                os.environ['ROUGE_TRACER_VERBOSE'] = env_value
                env_config = tracer._load_env_config()
    ...
    def test_logger_verbose_env_var(self):
        """Test that ROUGE_LOGGER_VERBOSE environment variable
        works correctly
        """
        ...
        for env_value, expected_bool in test_cases:
            with self.subTest(env_value=env_value, expected=expected_bool):
                os.environ['ROUGE_LOGGER_VERBOSE'] = env_value
                env_config = tracer._load_env_config()
