...
def log_verbose_error(config: RougeConfig, message: str, *args: Any) -> None:
    ...
    """
    if config.logger_verbose:
        print(f"[Rouge-Logger] ERROR: {message}", *args, file=sys.stderr)
...
class RougeLogger:
    ...
    def __init__(self,
        ...
        if (not self.config.local_mode
            ...
            # so we can still send logs to the rouge endpoint
...
_global_logger: RougeLogger | None = None
...
def initialize_logger(
    ...
    _global_logger = RougeLogger(config, credential_manager)
